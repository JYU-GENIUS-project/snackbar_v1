...
const createCategoryRequest = async ({ token, payload }) => {
  const response = await apiRequest({
    path: '/categories',
    method: 'POST',
    token,
...
const updateCategoryRequest = async ({ token, categoryId, payload }) => {
  const response = await apiRequest({
...
const deleteCategoryRequest = async ({ token, categoryId }) => {
  const response = await apiRequest({
    path: `/categories/${categoryId}`,
    method: 'DELETE',
    token
...
const updateCachedCategories = (queryClient, token, updater) => {
  queryClient.setQueryData([CATEGORIES_QUERY_KEY, token], (current) => {
    const base = Array.isArray(current) ? current : current?.data ?? [];
