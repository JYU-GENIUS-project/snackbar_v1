...
const buildAccountList = (mode) => {
    const desiredMode = mode === 'max' ? 'max' : 'default';
...
const AdminAccountsManager = () => {
    const [accounts, setAccounts] = useState(resolveInitialAccounts);
    ...
    const applySeed = useCallback((mode) => {
        const nextMode = mode === 'max' ? 'max' : 'default';
    ...
    const handleSelectAdmin = (accountId) => {
