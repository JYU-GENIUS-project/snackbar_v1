...
type FeedbackMessage = { type: 'success' | 'error' | 'info' | 'warning'; text: string } | string | null;
...
type InventoryPanelProps = {
  ...
  inventoryMetadata?: InventoryMetadataMap;
...
const deriveStatus = (
  item: InventoryItem,
  { isLowStock, hasDiscrepancy }: { isLowStock?: boolean; hasDiscrepancy?: boolean } = {}
) => {
...
const InventoryPanel = ({
  ...
  const apiSortBy = SORT_PARAM_MAP[sortBy as keyof typeof SORT_PARAM_MAP] || sortBy;

  const updateLocalItems = useCallback((updater: InventoryItem[] | ((items: InventoryItem[]) => InventoryItem[])) => {
  ...
  const latestInventoryDataRef = useRef<InventoryItem[] | null>(null);
  ...
  const items = useMemo(() => {
    ...
    if (inventoryQuery.data && Array.isArray(inventoryQuery.data.data)) {
      return inventoryQuery.data.data as InventoryItem[];
    }
  ...
  useEffect(() => {
    ...
    try {
      ...
      const pending = JSON.parse(raw) as InventoryItem | null;
      if (!pending) {
        return;
      }
  ...
  const sortedItems = useMemo(() => {
    ...
    const resolveSortValue = (item: InventoryItem, column: string) => {
      switch (column) {
        case 'stock':
        case 'current_stock':
          return (
            toFiniteNumber(item.currentStock ?? item.current_stock ?? item.stockQuantity ?? item.stock_quantity) ??
            -Infinity
          );
        case 'threshold':
        case 'low_stock_threshold':
          return (
            toFiniteNumber(item.lowStockThreshold ?? item.low_stock_threshold ?? (item as { threshold?: number }).threshold) ??
        ...
        case 'discrepancy':
        case 'ledger_balance':
          return Math.abs(toFiniteNumber(item.discrepancyTotal ?? item.discrepancy_total ?? item.ledger_balance) ?? 0);
        default:
  ...
  const applyLocalUpdate = (productKey: string | undefined, updater: (item: InventoryItem) => InventoryItem) => {
    if (!productKey) {
      return;
    }
  ...
  const handleSaveStock = async () => {
    ...
    if (shouldUseOffline) {
      applyLocalUpdate(productKey, (item) => {
        const thresholdValue = toFiniteNumber(item.lowStockThreshold ?? item.low_stock_threshold ?? null);
        const nextLowStock = thresholdValue !== null && parsedQuantity <= thresholdValue;
        return {
          ...item,
    ...
    try {
      ...
      setFeedbackMessage({
        type: 'error',
        text: error instanceof Error ? error.message : 'Unable to update stock level.'
      });
    }
  };
  ...
  const handleSaveAdjustment = async () => {
    ...
    try {
      ...
      setFeedbackMessage({ type: 'error', text: error instanceof Error ? error.message : 'Unable to adjust inventory.' });
    }
  };

  const renderStatusBadge = (item: InventoryItem, overrides: { isLowStock?: boolean; hasDiscrepancy?: boolean } = {}) => {
    ...
    <section className="card" id="inventory-management">
      ...
      {inventoryQuery.isError && token && (
        <div className="alert error" style={{ marginTop: '1rem' }}>
          ...
          {(inventoryQuery.error as Error | undefined)?.message || 'Please try again later.'}
        </div>
      )}
          ...
          <tbody>
            ...
            {sortedItems.map((item) => {
              ...
              const rawCurrentStock =
                item.currentStock ?? item.current_stock ?? item.stockQuantity ?? item.stock_quantity ?? null;
              ...
              const discrepancySource = item.discrepancyTotal ?? item.discrepancy_total ?? item.ledger_balance ?? 0;
              const discrepancyUnits = Math.abs(toFiniteNumber(discrepancySource) ?? 0);
              const hasDiscrepancy = Boolean(item.negativeStock ?? item.negative_stock) || discrepancyUnits > 0;
              const isLowStock =
              ...
              const decoratedItem = {
                ...
                lowStockThreshold: effectiveThreshold ?? item.lowStockThreshold ?? item.low_stock_threshold,
