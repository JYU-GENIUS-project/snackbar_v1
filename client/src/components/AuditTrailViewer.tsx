...
const AuditTrailViewer = ({ entries = [], onResetFilters }: AuditTrailViewerProps) => {
    ...
    return (
        <div className="stack">
                ...
                <form className="inline" style={{ gap: '1rem', flexWrap: 'wrap' }}>
                    <label className="stack" style={{ minWidth: '180px' }}>
                        ...
                        <select
                            ...
                            value={adminFilter}
                            onChange={(event: ChangeEvent<HTMLSelectElement>) => setAdminFilter(event.target.value)}
                        >
                    ...
                    <label className="stack" style={{ minWidth: '180px' }}>
                        ...
                        <select
                            ...
                            value={actionFilter}
                            onChange={(event: ChangeEvent<HTMLSelectElement>) => setActionFilter(event.target.value)}
                        >
                    ...
                    <label className="stack" style={{ minWidth: '160px' }}>
                        ...
                        <input
                            ...
                            onChange={(event: ChangeEvent<HTMLInputElement>) => setDateStartInput(event.target.value)}
                        />
                    </label>
                    ...
                    <label className="stack" style={{ minWidth: '160px' }}>
                        ...
                        <input
                            ...
                            type="date"
                            value={dateEndInput}
                            onChange={(event: ChangeEvent<HTMLInputElement>) => setDateEndInput(event.target.value)}
                        />
                    </label>

                    <div className="inline" style={{ alignItems: 'flex-end', gap: '0.75rem' }}>
                        <button id="apply-filters-button" className="button" type="button" onClick={applyDateFilters}>
                            Apply filters
                        </button>
                        <button className="button secondary" type="button" onClick={handleResetFilters}>
                            Reset
                        </button>
                    </div>
                </form>
            ...
            <section className="card" style={{ padding: '1rem' }}>
                ...
                {filteredLogs.length === 0 && <p className="helper">No audit entries match the selected filters.</p>}
            </section>
        </div>
    );
};
