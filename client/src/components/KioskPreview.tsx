...
type FeedProduct = {
  ...
  metadata?: { calories?: number | null } | null;
  primaryMedia?: { url?: string | null; alt?: string | null } | null;
};
...
const KioskPreview = () => {
  ...
  const typedData = data as FeedPayload | undefined;
  const products = typedData?.products ?? [];
  return (
    <section className="card" id="kiosk-preview">
      ...
      {error && (
        <div className="alert error">
          <span>{(error as Error).message || 'Unable to load product feed.'}</span>
        </div>
      )}
      ...
      {isLoading ? (
        ...
        <div style={gridStyles}>
          {products.map((product) => {
            const imageUrl = product.primaryMedia?.url ?? undefined;
            ...
            return (
              <article key={product.id} style={cardStyles}>
                ...
                <div>
                  ...
                  {product.metadata?.calories && <p className="helper">{product.metadata.calories} kcal</p>}
                  <div className="preview-cart-controls" style={{ marginTop: '0.5rem' }}>
                    <div className="cart-item-controls" style={{ gap: '0.5rem' }}>
                      ...
                      <span className="quantity-value cart-item-quantity">{hasLimit ? parsedLimit : ''}</span>
                      <button
                        type="button"
