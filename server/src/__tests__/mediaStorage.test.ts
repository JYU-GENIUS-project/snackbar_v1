...
describe('mediaStorage utility', () => {
    ...
    let mediaStorage: typeof import('../utils/mediaStorage');
    ...
    beforeAll(async () => {
        ...
        jest.resetModules();
        const loaded = await import('../utils/mediaStorage');
        mediaStorage = loaded.default ?? loaded;
    ...
    it('rejects unsupported variants', async () => {
        ...
        await expect(
            mediaStorage.persistMedia({
                productId,
                buffer: samplePng,
                variant: 'preview' as unknown as NonNullable<Parameters<typeof mediaStorage.persistMedia>[0]['variant']>
            })
