...
const { createImageProcessor, variantProfile } = imageProcessor;

type SharpBufferInfo = {
    data: Buffer;
    info: { width: number; height: number };
};

const createSharpMock = (buffers: SharpBufferInfo[]) => {
    const queue = [...buffers];
...
type StorageMock = {
    persistMedia: jest.Mock<
        ...
        [PersistMediaInput]
    >;
    deleteMedia: jest.Mock<Promise<void>, [string]>;
    ALLOWED_FORMATS: { webp: string; jpeg: string; png: string };
};

const createStorageMock = (): StorageMock => {
    const persistMedia = jest
        .fn<
            Promise<{
                storagePath: string;
                variant: string;
            ...
            [PersistMediaInput]
        >()
        .mockImplementation(({ variant, mimeType }: PersistMediaInput) =>
            Promise.resolve({
    ...
    const deleteMedia = jest.fn<Promise<void>, [string]>().mockResolvedValue(undefined);
